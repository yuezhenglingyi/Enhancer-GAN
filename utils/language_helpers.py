...
def load_dataset_1(max_length, max_n_examples, tokenize=False, max_vocab_size=2048, data_dir=''):
    '''
        Adapted from https://github.com/igul222/improved_wgan_training/blob/master/language_helpers.py
    '''
    print ("loading dataset...")# static
...
def load_dataset_2(data_dir, label=False):
    sequence_contents = []
    if label:
        with open(data_dir, "r") as data_file:
            ...
            for data_content in data_contents:
                ...
                sequence = items[0].replace("\n", "")
                label = float(items[1].replace("\n", ""))
                sequence_contents.append([sequence, label])
    else:
        with open(data_dir, "r") as data_file:
            data_contents = data_file.readlines()
            for data_content in data_contents:
                items = data_content.split("\t")
                sequence = items[0].replace("\n", "")
                sequence_contents.append([sequence, 0])
    ...
    return sequence_contents, charmap, inv_charmap

...
class ResBlock(nn.Module):
    def __init__(self, hidden, input_rate=1.0, output_rate=0.3):
    ...
    def forward(self, input):
        ...
        # return input + (0.3*output)  # original
        return (self.input_rate * input) + (self.output_rate * output)

class Generator_lang(nn.Module):
    def __init__(self, n_chars, seq_len, batch_size, hidden, input_rate=0.7, output_rate=0.3, gumbel=True):
        super(Generator_lang, self).__init__()
        self.fc1 = nn.Linear(128, hidden*seq_len)
        self.block = nn.Sequential(
            ResBlock(hidden, input_rate, output_rate),
            ResBlock(hidden, input_rate, output_rate),
            ResBlock(hidden, input_rate, output_rate),
            ResBlock(hidden, input_rate, output_rate),
            ResBlock(hidden, input_rate, output_rate),
...
class Discriminator_lang(nn.Module):
    def __init__(self, n_chars, seq_len, batch_size, hidden, input_rate=0.7, output_rate=0.3):
